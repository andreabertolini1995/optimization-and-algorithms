%Disks (with l2 regularizer)
%This file doesn't work because of line 23; I think it's because d^2 is not
%convex
...
cvx_begin quiet
   ...
   f1 = 0;
   for i = 1:6
       %d = max([(norm(E * x(:,tau(i)) - w(:,i)) - 2) 0]);   <- using max
       d = pos(norm(E * x(:,tau(i)) - w(:,i)) - 2);   % <- using pos
       f1 = f1 + d^2;
   end
